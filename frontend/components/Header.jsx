...
import { Refrigerator, Cookie, Sparkles } from "lucide-react";
          ...
          {user && (
            <PricingModal subscriptionTier = {user.subscriptionTier}>
              ...
              >
                <Sparkles 
                  className={`h-3 w-3 ${
                    user.subscriptionTier?.toLowerCase() === 'pro' ? 'text-white fill-white/20' : 'text-stone-500'}`} />
                  <span> {user.subscriptionTier?.toLowerCase() === 'pro' ? "Pro Chef" : "Free Plan"} </span>
              </Badge>
            </PricingModal>
          )}
          <UserDropdown />
